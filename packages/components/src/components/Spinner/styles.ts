...
const svgKeyframeNames: { [key: string]: string } = {
  ...
  smallEnterStroke: keyframes`
        from { stroke-dashoffset: ${getStrokeCircumference(
          SIZES_MAP.small,
        )}px; }
        to { stroke-dashoffset: ${
          getStrokeCircumference(SIZES_MAP.small) * 0.8
        }px; }
    `,
  mediumEnterStroke: keyframes`
        ...
        to { stroke-dashoffset: ${
          getStrokeCircumference(SIZES_MAP.medium) * 0.8
        }px; }
    `,
  largeEnterStroke: keyframes`
        ...
        to { stroke-dashoffset: ${
          getStrokeCircumference(SIZES_MAP.large) * 0.8
        }px; }
    `,
  xlargeEnterStroke: keyframes`
        ...
        to { stroke-dashoffset: ${
          getStrokeCircumference(SIZES_MAP.xlarge) * 0.8
        }px; }
    `,
};
...
const getSvgAnimation = (props: StyleParams) => {
  ...
  const animation = (animProps: StyleParams) => {
    ...
    return css`
      ${baseAnimation} ${svgKeyframeNames.rotate}
    `;
  };
...
export const getSvgStyles = (props: StyleParams) => getSvgAnimation(props);
